		...
		while (true) {
			...
			if (response.IsSuccessStatusCode) {
				if (response.Content.Headers.ContentDisposition?.FileName != null) {
					...
					var tmp = fileName;
					if (!tmp.Contains("!")) {//make a single file download appear in the same format as a multi-file download
						tmp = $"{tmp}.{fileSize}";
					}
					tmp = $"{tmp}!"; //a cheeky little move to make everything work with the code
					while (tmp.Contains("!")) { //this part of the code prepares the split points so we can split it into multiple files
						var exclamation_point = tmp.IndexOf("!");
						var dot_point = tmp.LastIndexOf(".", startIndex: exclamation_point);
						ulong fileRecordLength = ulong.Parse(
							tmp[(dot_point + 1)..(exclamation_point - 0)]
						);
					...
					while ((bytesRead = await stream.ReadAsync(buffer)) != 0) {
						int length_to_modify = (int)
							Math.Min(files[0].length_remaining, (ulong)bytesRead);
						files[0].length_remaining -= (ulong)length_to_modify;
						var slice = buffer[..length_to_modify];
						...
						if (files[0].length_remaining == 0) {
							...
							files[0].fs.Flush();
							files.RemoveAt(0);
							slice = buffer[length_to_modify..bytesRead];
							await files[0].fs.WriteAsync(slice);
							totalBytesRead = (ulong)slice.Count();
						}
					}
